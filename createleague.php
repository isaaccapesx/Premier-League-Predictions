...
$userid = $_SESSION['id'];
if (empty($leaguename)){
  $code = "Enter League name to find your code";
}
...
<head>
  ...
  <title>PLP | Create your league </title> <!--Title at the top so the user know which page they are on-->
</head>
<body>
    ...
    <div class="containernav flex"><!--class "containernav" is a box inside the header where all the content is. class "flex" makes the content change size when website is smaller -->
      ...
      <nav>
        <ul class="smA"> <!--sets the font size class-->
          ...
          <li><a href="Settings.php"><?php echo$_SESSION['username'];?></a></li><!-- links to faq page and is a button -->
        </ul>
      </nav>
    </div>
  </div>
  ...
  <div class="flex"><!-- class "flex" centres text and adjusts size -->
    <form action ="CodeGeneration.php" method="POST"> <!-- goes to codegeneration script -->
        <label for="LeagueName1"><div class ="LeagueName"><h1>Enter league name<h1></div></label> <!--allows users to enter their desired league name-->
        <input class ="input" type="text" id="LeagueName1" name="LeagueName1">
    </form>
  </div>
  <div class ="LeagueName flex">
    <h1>Your League code is : <?php echo$code; ?></h1>
  </div>
</div>
    ...
    <div class="containernav grid grid-3"><!--class "containernav" is a box inside the footer where all the content is. class "grid" is a grid in the footer for each individual section. class "grid-3" is space for each individual section-->
      ...
      <nav> <!--section for links/buttons-->
        <ul class="footernav smA">  <!--class "footernav" that sets it to the right. class "smA" is a font size-->
          ...
          <li><a class="active" href="YourPredictions.php">Your Predictions</a></li>  <!--footer your prediction buttons-->
